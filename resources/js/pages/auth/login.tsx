...
export default function Login({ status, canResetPassword }: LoginProps) {
    ...
    const submit: FormEventHandler = (e) => {
        ...
        post('/login', {
            ...
            onError: (errors) => {
                // If we get a 419 error (CSRF token mismatch), refresh the page to get a new token
                if (errors && typeof errors === 'object' && 'message' in errors) {
                    const errorMessage = errors.message as string;
                    if (errorMessage && errorMessage.includes('419') || errorMessage.includes('expired') || errorMessage.includes('CSRF')) {
